...
class ShakeDetector(private val listener: Listener) : SensorEventListener {
     ...
     */
    @JvmOverloads
    fun start(sensorManager: SensorManager, sensorDelay: Int = SensorManager.SENSOR_DELAY_FASTEST): Boolean {
        // Already started?
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (queue.isShaking) {
            queue.clear()
            listener.hearShake(SystemClock.elapsedRealtime())
        }
    }
    ...
    internal class SampleQueue {
        ...
        private fun purge(cutoff: Long) {
            while (sampleCount >= MIN_QUEUE_SIZE && oldest != null && cutoff - (oldest?.timestamp ?: 0) > 0) {
                // Remove sample.
                val removed: Sample = oldest as Sample
        ...
        val isShaking: Boolean
             ...
             * are accelerating.
             */
            get() = newest != null && oldest != null && (newest?.timestamp ?: 0) - (oldest?.timestamp
                ?: 0) >= MIN_WINDOW_SIZE && acceleratingCount >= (sampleCount shr 1) + (sampleCount shr 2)
    ...
    companion object {
        private const val DEFAULT_ACCELERATION_THRESHOLD = 11
    }
}
